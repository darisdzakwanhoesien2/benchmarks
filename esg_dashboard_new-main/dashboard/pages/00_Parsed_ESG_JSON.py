...
# Page Config
# -------------------------------------------------------
st.set_page_config(page_title="Parsed ESG JSON Parser", layout="wide")
st.title("📊 ESG Parsed Sentence-Level Dashboard")
st.markdown(
    """
This page parses raw model output into structured sentence-level ESG records.

**What this page does**
- extracts JSON blocks from raw model responses
- normalizes nested JSON into flat records
- validates sentence-level aspect annotations for downstream analysis

**What this page expects**
- the resolved `data_output` dataset with a `text` column containing model output

**How to use it**
- use this page as a first-pass audit of parsing quality
- check the parsed record count before moving into comparison or distribution pages
- investigate strange outputs here if downstream pages look incomplete
"""
)
...
for col in ["filename", "model"]:
    if col in raw_df.columns:
        raw_df[col] = raw_df[col].map(format_display_value)
