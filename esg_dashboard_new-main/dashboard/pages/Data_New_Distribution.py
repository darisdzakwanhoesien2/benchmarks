...
st.markdown(
    """
This page is the main ontology-aware exploration workspace for the resolved `output_in_csv` dataset.

**What this page does**
- normalizes aspect category, sentiment, and tone using local ontologies
- builds a global Sankey view from aspect to sentiment to tone
- lets you inspect records with waterfall filters and multi-rule sampling

**What this page expects**
- the resolved `output_in_csv` dataset in either `.csv` or `.txt` form
- ontology JSON files in `dashboard/data`

**How to use it**
- start with the Sankey chart to understand the high-level flow
- narrow the dataset with the waterfall filter for focused review
- use the multi-rule builder when you want to extract specific slices of the dataset
"""
)
