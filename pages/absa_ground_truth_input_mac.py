...
st.title("ABSA Ground Truth Input (Mac)")

PAGE_DIR = Path(__file__).resolve().parent
BENCHMARK_ROOT = PAGE_DIR.parent
GROUND_TRUTH_PATH = BENCHMARK_ROOT / "data" / "ground_truth" / "absa_mapping.csv"

st.caption(f"Source used by `absa_metrics_comparison_mac.py`: `{GROUND_TRUTH_PATH}`")
...
with col2:
    st.download_button(
        ...
        file_name="absa_mapping_mac_edited.csv",
        mime="text/csv",
        use_container_width=True,
    )
...
st.data_editor(core_rows, num_rows="dynamic", use_container_width=True, key="gt_guide_mac_core")
...
st.data_editor(support_rows, num_rows="dynamic", use_container_width=True, key="gt_guide_mac_support")
