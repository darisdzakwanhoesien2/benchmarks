...
```mermaid
flowchart TD
  A[PDF sustainability and annual reports] --> B[OCR and page extraction]
  B --> C[Page markdown, images, OCR JSON]
  C --> D[LLM ESG extraction]
  D --> E[Structured record store: esg_records.json]
  E --> F[T1 ClimateBERT or local comparison]
  E --> G[T2 ABSA and tone processing]
  F --> H[Agreement and validation tables]
  G --> H
  H --> I[Ontology alignment and diagnostics]
  I --> J[Dashboards, CSV/JSON exports, thesis artifacts]
```
